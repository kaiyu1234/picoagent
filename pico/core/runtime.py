...
class Pico(RuntimeSecretsMixin, RuntimeCheckpointsMixin):
    ...
    def reset_abort_state(self):
        self.abort_requested = False
        reset_abort = getattr(self.model_client, "reset_abort", None)
        if callable(reset_abort):
            reset_abort()

    def detach_aborted_model_client(self, aborted_client):
        if self.model_client is not aborted_client:
            return True
        factory = self.model_client_factory
        clone = getattr(aborted_client, "clone", None)
        try:
            replacement = clone() if callable(clone) else factory()
        except Exception:
            return False
        self.model_client = replacement
        self.model_client_router.replace_default_client(aborted_client, replacement)
        return True

    def abort_current_turn(self):
        if not getattr(self, "current_run_id", ""):
            return False
        if self.abort_requested:
            return True
        self.abort_requested = True
        self.session_event_bus.emit(
            "abort_requested", {"run_id": self.current_run_id}
        )
        ...
        if callable(abort):
            try:
                ...
                pass
        abort_shell = getattr(self.sandbox_runner, "abort", None)
        if callable(abort_shell):
            try:
                abort_shell()
            except Exception:
                pass

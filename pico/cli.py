...
def _print_repl_event(event):
    event_type = str(event.get("type", ""))
    if event_type == "model_requested":
        attempts = int(event.get("attempts", 0) or 0)
        tool_steps = int(event.get("tool_steps", 0) or 0)
        print(
            f"[thinking] model request {attempts} (tools completed: {tool_steps})...",
            flush=True,
        )
        return
    if event_type == "model_parsed":
        kind = str(event.get("kind", "response") or "response")
        duration = _format_duration(event.get("duration_ms", 0))
        print(f"[model] received {kind} response in {duration}", flush=True)
        return
    if event_type == "model_retry":
        code = str(event.get("code", "model_error") or "model_error")
        retry_count = int(event.get("retry_count", 0) or 0)
        print(f"[retry] provider {code}; retry {retry_count}", flush=True)
        return
    if event_type == "tool_call":
        name = str(event.get("name", "tool") or "tool")
        print(f"[tool] {name} started", flush=True)
        return
    if event_type == "tool_result":
        name = str(event.get("name", "tool") or "tool")
        metadata = event.get("metadata")
        metadata = metadata if isinstance(metadata, dict) else {}
        status = str(metadata.get("tool_status", "done") or "done")
        duration = _format_duration(event.get("duration_ms", 0))
        print(f"[tool] {name} {status} in {duration}", flush=True)
        return
    if event_type == "worker_notification":
        ...
        return
    if event_type == "retry":
        print(f"[retry] {event.get('content', '')}", flush=True)
        return
    if event_type == "runtime_notice":
        print(str(event.get("content", "")), flush=True)
        return
    if event_type in {"final", "stop"}:
        print(str(event.get("content", "")), flush=True)


def _run_repl_turn(agent, user_input):
    final_answer = ""
    for event in agent.engine.run_turn(user_input):
        _print_repl_event(event)
        if event.get("type") in {"final", "stop"}:
            final_answer = str(event.get("content", ""))
    return final_answer


def _join_repl_thread(thread, timeout):
    thread.join(timeout)


def _run_repl_turn_interruptible(agent, user_input):
    outcome = {}

    def drive_turn():
        try:
            outcome["answer"] = _run_repl_turn(agent, user_input)
    ...
    while thread.is_alive():
        try:
            _join_repl_thread(thread, 0.1)
        except KeyboardInterrupt:
            if stopping:
                print("\n[abort] forcing Pico to exit.", flush=True)
                return True
            if agent.abort_current_turn():
                stopping = True
                print("\n[abort] stopping current turn...", flush=True)
...
def main(argv=None):
    ...
    while True:
        ...
        print()
        try:
            if _run_repl_turn_interruptible(agent, user_input):
                return 130
        except RuntimeError as exc:
            print(str(exc), file=sys.stderr)

...
class ModelRequestAborted(RuntimeError):
    """Raised when a user closes an active provider response."""
    ...
    for attempt in range(attempts):
        ...
        try:
            with urllib.request.urlopen(request, timeout=timeout) as response:
                if set_active_response is not None:
            ...
            _raise_if_aborted(abort_event, cause=exc)
            body = exc.read().decode("utf-8", errors="replace")
            retryable = exc.code in RETRYABLE_HTTP_STATUS or exc.code >= 500
            if retryable and attempt < attempts - 1:
                retry_count += 1
                _wait_retry_delay(abort_event, _retry_delay(attempt, exc.headers))
                continue
            raise ProviderError(
            ...
            if attempt < attempts - 1:
                retry_count += 1
                _wait_retry_delay(abort_event, _retry_delay(attempt, None))
                continue
            raise ProviderError(
...
def _raise_if_aborted(abort_event, cause=None):
    if abort_event is not None and abort_event.is_set():
        error = ModelRequestAborted("model request aborted")
        if cause is not None:
            raise error from cause
        raise error


def _wait_retry_delay(abort_event, delay):
    if abort_event is None:
        time.sleep(delay)
        return
    if abort_event.wait(delay):
        raise ModelRequestAborted("model request aborted")
...
class _AbortableModelClient:
    def _init_abort_state(self):
    ...
    def _request(self, provider, request):
        return _request_with_retries(
            ...
            set_active_response=self._set_active_response,
        )


class OpenAICompatibleModelClient(_AbortableModelClient):
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        try:
            body_text, content_type, request_metadata = self._request("openai", request)
...
class AnthropicCompatibleModelClient(_AbortableModelClient):
    def __init__(self, model, base_url, api_key, temperature, timeout):
        self.model = model
        self.base_url = _normalize_versioned_base_url(base_url)
        self.api_key = api_key
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        try:
            body_text, _content_type, request_metadata = self._request(
                "anthropic", request
            )
        except ProviderError as exc:
            self.last_completion_metadata = exc.to_metadata()

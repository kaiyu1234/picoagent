...
class PicoTuiApp(App):
    ...
    BINDINGS = [
        ...
        Binding("ctrl+c", "interrupt", "Abort / Quit", priority=True),
    ...
    def __init__(self, agent, **kwargs) -> None:
        ...
        self._abort_requested_by_user = False
    ...
    def action_interrupt(self) -> None:
        if not self._turn_is_busy():
            self.exit()
            return
        if self._abort_requested_by_user:
            self.exit(return_code=130)
            return
        if not self.agent.abort_current_turn():
            return
        self._abort_requested_by_user = True
        self._release_pending_prompts()
        self.query_one(ThinkingIndicator).set_detail("stopping current turn")
        self.query_one(ChatLog).add_message(
            "assistant", "[abort] stopping current turn..."
        )
    ...
    def _handle_runtime_event(self, event: dict) -> None:
        ...
        if event_type == "model_retry":
            code = event.get("code", "model_error")
            retry_count = event.get("retry_count", 0)
            self.query_one(ThinkingIndicator).set_detail(
                f"retrying {code} ({retry_count})"
            )
            return
    ...
    def _turn_is_busy(self) -> bool:
        try:
            return bool(self.query_one(InputBar).input.disabled)
        except NoMatches:
            return bool(getattr(self.agent, "current_run_id", ""))

    def _release_pending_prompts(self) -> None:
        if self._confirm_decision is not None:
            self._resolve_confirm(False)
        if self._ask_user_decision is not None:
            self._resolve_ask_user("")
